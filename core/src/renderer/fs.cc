...
V8Value* native_Stat(const vec<V8Value*>& args) {
    ...
    v8Obj->set(&u"length"_s, V8Value::number(fileStat.size), V8_PROPERTY_ATTRIBUTE_READONLY);
    v8Obj->set(&u"isDir"_s, V8Value::boolean(fileStat.isDir), V8_PROPERTY_ATTRIBUTE_READONLY);
    v8Obj->set(&u"fileName"_s, V8Value::string(&CefStr{ fileStat.fileName }), V8_PROPERTY_ATTRIBUTE_READONLY);
